(function ($) {
	//plot graph
	createPlot();
	//plot graph Dot graph
	createPlotDot();
	//Pie graph graph
	createPie();
	//real time representation graph
	createRealTime();
	//Donut pie graph
	createDonut();
	//Stack Bar graph
...
function createPlot() {
	...
	createPlotGraph("#website-stats", uploads, downloads, plabels, chartColors, borderColor, bgColor);
}
function createPlotGraph(selector, data1, data2, labels, colors, borderColor, bgColor) {
	$.plot($(selector), [{
		data: data1,
		label: labels[0],
		color: colors[0]
...
function createPlotDot() {
	...
	createPlotDotGraph("#website-stats1", uploadsDot, downloadsDot, plabelsDot, chartColors, borderColorDot, bgColorDot);
}
function createPlotDotGraph(selector, data1, data2, labelsDot, colorsDot, borderColorDot, bgColorDot) {
	$.plot($(selector), [{
		data: data1,
		label: labelsDot[0],
		color: colorsDot[0]
...
function createPie() {
	...
	var chartColors = [colors.primary, colors.danger, colors.success, colors.info, colors.warning];
	createPieGraph("#pie-chart", pielabels, datas, chartColors);
}

function createPieGraph(selector, labels, datas, colors) {
...
function createRealTime() {
	...
	var plot = createRealTimeGraph('#flotRealTime', randomData(), [chartColors]);
	plot.draw();
	function updatePlot() {
...
function createDonut() {
	...
	var chartColors = [colors.primary, colors.danger, colors.success, colors.warning, colors.info];
	createDonutGraph("#donut-chart", donutlabels, donutdatas, chartColors);
}
...
function createStackBar() {
	...
	var chartColors = [colors.primary, colors.danger, colors.gray400];
	createStackBarGraph("#ordered-bars-chart", stack_ticks, chartColors, ds);
}
function createStackBarGraph(selector, ticks, colors, data) {
	var options = {
		bars: {
			show: true,
			barWidth: 0.2,
...
function createLine() {
	...
	var chartColors = [colors.primary, colors.danger];
	createLineGraph("#line-chart-alt", line_ticks, chartColors, line_data);
}
...
function createCombine() {
	...
	var chartColors = [colors.grayDark, colors.warning, colors.danger];
	var dataColors = $("#combine-chart").data('colors');
	if (dataColors) {
		colors = dataColors.split(",");
	}
	createCombineGraph("#combine-chart", ticks, combinelabels, combinedatas, chartColors);
}
function createCombineGraph(selector, ticks, labels, datas, colors) {
	var data = [{
		label: labels[0],
		data: datas[0],
		color: colors[0],
